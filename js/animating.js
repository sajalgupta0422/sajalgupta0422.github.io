    ...
    function ajaxLoader() {
        ...
        var href = $('.ajax-page-load').each(function(){
            ...
            if(location.hash == location.hash.split('/')[0] + '/' + href.substr(0,href.length-5)){
                ...
                showContent();
                //ajaxLoadedContent.load(toLoad); Sajal Changed to the below to fix caching issue
                ajaxLoadedContent.load(toLoad, function () {
                // Re-typeset MathJax for AJAX-injected pages
                if (window.MathJax && MathJax.typesetPromise) {
                    // Wait for MathJax startup (safe even if already ready)
                    var p = (MathJax.startup && MathJax.startup.promise) ? MathJax.startup.promise : Promise.resolve();
                    p.then(function () {
                        MathJax.typesetPromise([ajaxLoadedContent.get(0)]);
                    });
                }
            });
                return false;
